...
contract('Otoken', ([deployer, controller, user1, user2, random]) => {
  ...
  describe('Otoken Initialization', () => {
    it('should be able to initialize with put parameter correctly', async () => {
      ...
      bool _isPut
      */
      await otoken.init(addressBookAddr, weth.address, usdc.address, [usdc.address], ['0'], strikePrice, expiry, isPut, {
        from: deployer,
      })
    ...
    it('should get correct otoken details', async () => {
      ...
      */
      ...
      assert.equal(otokenDetails[0][0], usdc.address, 'getOtokenDetails collateralAssets mismatch')
      assert.equal(otokenDetails[1][0].toString(), '0', 'getOtokenDetails collateralsAmounts must be zero')
      assert.equal(otokenDetails[2][0].toString(), '0', 'getOtokenDetails collateralsValues must be zero')
      assert.equal(otokenDetails[3][0].toString(), '6', 'getOtokenDetails collateralsDecimals must be zero')      
      assert.equal(otokenDetails[4].toString(), weth.address, 'getOtokenDetails underlyingAsset mismatch')
      assert.equal(otokenDetails[5].toString(), usdc.address, 'getOtokenDetails strike asset mismatch')
      assert.equal(otokenDetails[6].toString(), strikePrice.toString(), 'getOtokenDetails strike price mismatch')
      assert.equal(otokenDetails[7].toString(), expiry.toString(), 'getOtokenDetails expiry mismatch')
      assert.equal(otokenDetails[8], isPut, 'getOtokenDetails isPut mismatch')
      assert.equal(otokenDetails[9].toString(), '0', 'getOtokenDetails collaterizedTotalAmount must be zero')
    })

    it('should revert when init is called again with the same parameters', async () => {
      await expectRevert(
        otoken.init(addressBookAddr, weth.address, usdc.address, [usdc.address], ['0'], strikePrice, expiry, isPut),
    ...
    it('should revert when init is called again with different parameters', async () => {
      await expectRevert(
        otoken.init(addressBookAddr, usdc.address, weth.address, [weth.address], ['0'], strikePrice, expiry, false),
        'Initializable: contract is already initialized',
      )
    })
    ...
    it('should set the right name for calls', async () => {
      ...
      await callOption.init(addressBookAddr, weth.address, usdc.address, [weth.address], ['0'], strikePrice, expiry, false, {
        from: deployer,
      })
    ...
    it('should set the right name for multicollateral otokens', async () => {
      const callOption = await Otoken.new()
      await callOption.init(addressBookAddr, weth.address, usdc.address, [weth.address, usdc.address], ['0', '0'], strikePrice, expiry, false, {
        from: deployer,
      })
    ...
    it('should set the right name for option with strikePrice 0', async () => {
      ...
      await oToken.init(addressBookAddr, weth.address, usdc.address, [usdc.address], ['0'], strikePrice, expiry, true, {
        from: deployer,
      })
    ...
    it('should set the right name for non-eth options', async () => {
      ...
      await putOption.init(addressBookAddr, weth.address, usdc.address, [usdc.address], ['0'], strikePrice, expiry, isPut, {
        from: deployer,
      })
    ...
    it('should set the right name for options with 0 expiry (should be banned by factory)', async () => {
      /* This behavior should've been banned by factory) */
      const otoken = await Otoken.new()
      await otoken.init(addressBookAddr, weth.address, usdc.address, [usdc.address], ['0'], strikePrice, 0, isPut, {
        from: deployer,
      })
    ...
    it('should set the right name for options expiry on 2345/12/31', async () => {
      ...
      await otoken.init(addressBookAddr, weth.address, usdc.address, [usdc.address], ['0'], strikePrice, _expiry, isPut, {
        from: deployer,
      })
    ...
    it('should set the right name and symbol for expiry on each month', async () => {
      ...
      await January.init(addressBookAddr, weth.address, usdc.address, [usdc.address], ['0'], strikePrice, 1893456000, isPut, {
        from: deployer,
      })
      ...
      await February.init(addressBookAddr, weth.address, usdc.address, [usdc.address], ['0'], strikePrice, 1896134400, isPut, {
        from: deployer,
      })
      ...
      await March.init(addressBookAddr, weth.address, usdc.address, [usdc.address], ['0'], strikePrice, 1898553600, isPut, {
        from: deployer,
      })
      ...
      await April.init(addressBookAddr, weth.address, usdc.address, [usdc.address], ['0'], strikePrice, 1901232000, isPut, {
        from: deployer,
      })
      ...
      await May.init(addressBookAddr, weth.address, usdc.address, [usdc.address], ['0'], strikePrice, 1903824000, isPut, {
        from: deployer,
      })
      ...
      await June.init(addressBookAddr, weth.address, usdc.address, [usdc.address], ['0'], strikePrice, 1906502400, isPut, {
        from: deployer,
      })
      ...
      await July.init(addressBookAddr, weth.address, usdc.address, [usdc.address], ['0'], strikePrice, 1909094400, isPut, {
        from: deployer,
      })
      ...
      await August.init(addressBookAddr, weth.address, usdc.address, [usdc.address], ['0'], strikePrice, 1911772800, isPut, {
        from: deployer,
      })
      ...
      await September.init(addressBookAddr, weth.address, usdc.address, [usdc.address], ['0'], strikePrice, 1914451200, isPut, {
        from: deployer,
      })
      ...
      await October.init(addressBookAddr, weth.address, usdc.address, [usdc.address], ['0'], strikePrice, 1917043200, isPut, {
        from: deployer,
      })
      ...
      await November.init(addressBookAddr, weth.address, usdc.address, [usdc.address], ['0'], strikePrice, 1919721600, isPut, {
        from: deployer,
      })
      ...
      await December.init(addressBookAddr, weth.address, usdc.address, [usdc.address], ['0'], strikePrice, 1922313600, isPut, {
        from: deployer,
      })
    ...
    it('should display strikePrice as $0 in name and symbol when strikePrice < 10^18', async () => {
      ...
      await testOtoken.init(addressBookAddr, weth.address, usdc.address, [usdc.address], ['0'], 0, expiry, isPut, {
        from: deployer,
      })
