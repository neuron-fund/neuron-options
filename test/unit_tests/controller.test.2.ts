...
contract('Controller', ([owner, user, seller, redeemer]) => {
  // ERC20 mock
  ...
  let longCall: MockONtokenInstance
  ...
  before('Deployment', async () => {
    ...
    longCall = await MockONtoken.new()
    ...
    await longCall.init(addressBook.address, weth.address, usdc.address, collaterals, [0], strike, expiry, false)

    await whitelist.whitelistONtoken(longCall.address, { from: owner })
    assert.isTrue(await whitelist.isWhitelistedONtoken(longCall.address))
  ...
  describe('Long onToken', () => {
    it('should revert if long and short has different underlyings', async () => {
      const shortCall: MockONtokenInstance = await MockONtoken.new()
      ...
      await whitelist.whitelistONtoken(shortCall.address, { from: owner })
      assert.isTrue(await whitelist.isWhitelistedONtoken(shortCall.address))
      assert.isTrue(await whitelist.isWhitelistedONtoken(longCall.address))
      ...
      const actionArgs = [
        getAction(ActionType.OpenVault, {
          owner: user,
          shortONtoken: shortCall.address,
          vaultId: vaultId,
        }),
    ...
    it('should free long when burn short collaterated by this long', async () => {
      ...
      const shortCall: MockONtokenInstance = await MockONtoken.at(currenVault.shortONtoken)
      const mintedShorts = await shortCall.balanceOf(user)
      ...
      const proportionLongs =
        Number(currenVault.usedLongAmount.toString()) / Number(afterBurnVault.usedLongAmount.toString())
    ...
    it('should free long and collaterals when burn short if long strike < short strike', async () => {
      ...
      assert.strictEqual(
        ...
        createTokenAmount(5, wethDecimals - 1)
      )
  ...
  describe('Redeem', () => {
    ...
    it('should redeem from undercollaretized call', async () => {
      ...
      assert.isTrue(curTime > expiryTimestamp, "Short onToken hasn't expired")
    ...
    it('should redeem from undercollaretized put', async () => {
      ...
      const shortPut: MockONtokenInstance = await MockONtoken.new()
      const strike = createTokenAmount(100)
      ...
      await whitelist.whitelistONtoken(shortPut.address, { from: owner })
      assert.isTrue(await whitelist.isWhitelistedONtoken(shortPut.address))
