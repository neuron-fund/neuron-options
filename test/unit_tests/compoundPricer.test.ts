import { 
  ...
  MockCToken as MockCTokenInstance,
  CompoundPricer as CompoundPricerInstance,
...
contract('CompoundPricer', ([owner, random]) => {
  ...
  describe('getPrice for cETH', () => {
    ...
    const exchangeRate = BigNumber.from('200192735438752381581313918')
    ...
    it('should return the price in 1e8', async () => {
      ...
      const ethPriceB =  BigNumber.from(ethPrice)
      const expectResult = await underlyingPriceToCtokenPrice(ethPriceB, exchangeRate, weth)
    ...
    it('should return the new price after resetting answer in underlying pricer', async () => {
      ...
      const cTokenPrice = await cethPricer.getPrice()
      const expectedResult = await underlyingPriceToCtokenPrice(BigNumber.from(newPrice), exchangeRate, weth)
  ...
  describe('getPrice for cUSDC', () => {
    const usdPrice = BigNumber.from(createScaledNumber(1))
    const exchangeRate = BigNumber.from('211619877757422')
    ...
    it('should return the price in 1e8', async () => {
      ...
      const expectResult = await underlyingPriceToCtokenPrice(BigNumber.from(usdPrice), exchangeRate, usdc)
      assert.equal(cTokenprice.toString(), expectResult.toString())
      // hard coded answer
      // 1 cUSDC = 0.02 USD
    ...
    it('should return the new price after resetting answer in underlying pricer', async () => {
      ...
      const newPrice = createScaledNumber(11, 7) // 1.1e8 = 11e7
      console.log('newPrice', newPrice)
      await oracle.setStablePrice(usdc.address, newPrice)
      const cTokenPrice = await cusdcPricer.getPrice()
      const expectedResult = await underlyingPriceToCtokenPrice(BigNumber.from(newPrice), exchangeRate, usdc)
      assert.equal(cTokenPrice.toString(), expectedResult.toString())
    })
  })
