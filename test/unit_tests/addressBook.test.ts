...
import { 
  ...
  let owner: string, otokenImplAdd: string, marginPoolAdd: string, random: string
  ...
  describe('AddressBook', () => {
    ...
    before('Deployment', async () => {
      ...
      owner = deployer
      otokenImplAdd = random_user
      marginPoolAdd = random_user2
      random = random_user3
    ...
    describe('Set controller', () => {
      ...
      it('should set controller address', async () => {
        ...
        const proxy: OwnedUpgradeabilityProxyInstance = OwnedUpgradeabilityProxy.attach(await addressBook.getController()) as OwnedUpgradeabilityProxyInstance
    ...
    describe('Set otoken factory', () => {
      ...
      it('should revert adding otoken factory address from non-owner address', async () => {
        await expect(
          addressBook.connect(random).setOtokenFactory(otokenFactory.address)).to.be.revertedWith(
          'Ownable: caller is not the owner',
        )
      })
    ...
    describe('Set whitelist', () => {
      ...
      it('should revert adding whitelist address from non-owner address', async () => {
        await expect(
          addressBook.connect(random).setWhitelist(whitelist.address)).to.be.revertedWith(
          'Ownable: caller is not the owner',
        )
      })
    ...
    describe('Set oracle', () => {
      ...
      it('should revert adding oracle address from non-owner address', async () => {
        await expect(
          addressBook.connect(random).setLiquidationManager(oracle.address)).to.be.revertedWith(
          'Ownable: caller is not the owner',
        )
      })
    ...
    describe('Set margin calculator', () => {
      ...
      before(async () => {
        ...
        marginCalculator = await (await ethers.getContractFactory("MarginCalculator",{
            libraries: {
              "ArrayAddressUtils": arrayAddressUtils.address,
            },
      ...
      it('should revert adding margin calculator address from non-owner address', async () => {
        await expect(
          addressBook.connect(random).setMarginCalculator(marginCalculator.address)).to.be.revertedWith(
          'Ownable: caller is not the owner',
        )
      })
    ...
    describe('Set arbitrary address', () => {
      ...
      before(async () => {
        module = await (await ethers.getContractFactory("UpgradeableContractV1")).deploy() as UpgradeableContractV1Instance
    ...
    describe('Upgrade contract', async () => {
      ...
      it('should upgrade contract from V1 to V2', async () => {
        ...
        v2Contract = UpgradeableContractV2.attach(            
            await addressBook.getAddress(key),
          ) as UpgradeableContractV2Instance
